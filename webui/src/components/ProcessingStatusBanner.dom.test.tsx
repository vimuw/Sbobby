...
describe('ProcessingStatusBanner', () => {
  ...
  it('shows file counter chip when currentBatchTotal > 1', () => {
    render(
      <ProcessingStatusBanner
        ...
        workTotals={{ chunks: 3, macro: 3, boundary: 2 }}
        currentFileIndex={1}
        currentBatchTotal={3}
        currentFileName="lesson.mp3"
  ...
  it('shows wait banner for Modello non disponibile phase', () => {
    render(
      <ProcessingStatusBanner
        ...
        currentModel="gemini-2.5-flash"
        activeProgress={30}
        workDone={{ chunks: 2, macro: 0, boundary: 0 }}
        workTotals={{ chunks: 6, macro: 6, boundary: 5 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
      />,
  ...
  it('shows pause banner for Rate limit phase', () => {
    render(
      <ProcessingStatusBanner
        ...
        currentModel="gemini-2.5-flash"
        activeProgress={40}
        workDone={{ chunks: 3, macro: 0, boundary: 0 }}
        workTotals={{ chunks: 6, macro: 6, boundary: 5 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
  ...
  it('shows all-done stepper for __completed__ phase', () => {
    render(
      <ProcessingStatusBanner
        ...
        currentModel="gemini-2.5-flash"
        activeProgress={100}
        workDone={{ chunks: 6, macro: 6, boundary: 5 }}
        workTotals={{ chunks: 6, macro: 6, boundary: 5 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
  ...
  it('shows macro progress from (blocco X/Y) regex in Fase 2/3', () => {
    render(
      <ProcessingStatusBanner
        ...
        currentModel="gemini-2.5-flash"
        activeProgress={40}
        workDone={{ chunks: 6, macro: 3, boundary: 0 }}
        workTotals={{ chunks: 6, macro: 8, boundary: 7 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
      />,
  ...
  it('renders without progress label when phase does not match any known pattern', () => {
    ...
    expect(screen.queryByText(/Blocco|Sezione|Confine/)).toBeNull();
  });

  it('shows boundary progress from regex when phase includes count', () => {
    render(
      <ProcessingStatusBanner
        appState="processing"
        currentPhase="Fase 3/3: confini (5/10)"
        currentModel="gemini-2.5-flash"
        activeProgress={50}
        workDone={{ chunks: 10, macro: 10, boundary: 5 }}
        workTotals={{ chunks: 10, macro: 10, boundary: 10 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
      />,
    );
    expect(screen.getByText('Confine 5 di 10')).toBeTruthy();
  });
  ...
  it('shows Fase 1/3 progress label when no chunk number in phase', () => {
    render(
      <ProcessingStatusBanner
        ...
        activeProgress={10}
        workDone={{ chunks: 0, macro: 0, boundary: 0 }}
        workTotals={{ chunks: 5, macro: 5, boundary: 4 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
      />,
  ...
  it('shows Fase 3/3 progress from active step when no boundary number in phase', () => {
    render(
      <ProcessingStatusBanner
        appState="processing"
        currentPhase="Fase 3/3: confini"
        currentModel="gemini-2.5-flash"
        activeProgress={30}
        workDone={{ chunks: 10, macro: 10, boundary: 2 }}
        workTotals={{ chunks: 10, macro: 10, boundary: 8 }}
        currentFileIndex={0}
        currentBatchTotal={1}
        currentFileName="lesson.mp3"
      />,
    );
    expect(screen.getByText(/Confine \d+ di 8/)).toBeTruthy();
  });
  ...
  it('shows hours in elapsed chip when startedAt is > 1 hour ago', () => {
    render(
      <ProcessingStatusBanner
        ...
        currentModel="gemini-2.5-flash"
        activeProgress={50}
        workDone={{ chunks: 10, macro: 5, boundary: 0 }}
        workTotals={{ chunks: 10, macro: 10, boundary: 8 }}
        currentFileIndex={0}
        currentBatchTotal={1}
